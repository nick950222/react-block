...
function serverHandler(request, response) {
    ...
    try {
        ...
        try {
            ...
            console.log(uri)
            filename = path.join(config.dirPath ? resolveURL(config.dirPath) : process.cwd(), 'build',ur);
        ...
        var matched = false;
        ['/socket.io/','/build', '/node_modules/canvas-designer/'].forEach(function(item) {
        ...
        try {
            ...
            if (filename.search(/build/g) === -1 && filename.search(/admin/g) === -1 && stats.isDirectory() && config.homePage === '/build/index.html') {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                } else if (filename.indexOf(resolveURL('/build/')) !== -1) {
                    filename = filename.replace(resolveURL('/build/'), '');
                    filename += resolveURL('/build/index.html');
                }  else {
